...
def get_symbol_meta(symbol: str, cache: Dict[str, dict]) -> Tuple[Optional[str], Optional[str], Optional[str], Optional[str]]:
    ...
    try:
        ...
        try:
            ...
            if fi:
                fast_info_name = fi.get("shortName") if isinstance(fi, dict) else None
        except Exception:
            fast_info_name = None
...
def build_candidate_for_account(
    ...
    if df is None or df.empty or len(df) < max(120, breakout_days + 5):
        return None
