...
# - 순수 가격 모멘텀 돌파
# - 핵심 = 돌파 + RS + 거래량
# - 시장 레짐/브레드스/산업 강도는 "해석 및 우선순위"에 사용
# - 후보는 시장이 약세여도 항상 출력
#
# 이번 버전 반영
# 1) 유니버스 축소
#    - ETF 제외
#    - 가격 10달러 이상
#    - 20일 평균 거래대금 2천만 달러 이상
#    - 시가총액 10억 달러 이상
#    - 바이오 / 제약 / 중국 ADR / SPAC 계열 제외
# 2) 52주 신고가 근접 필터 추가
# 3) 계좌 A = 55일 돌파 / 계좌 B = 20일 돌파
# 4) 순수 모멘텀 돌파: 종가 > 50일선만 유지
# ============================================================
...
# 52주 신고가 근접
MIN_HIGH_52W_PROXIMITY = 0.85   # 최소 52주 신고가의 85% 이상
PREFERRED_HIGH_52W_PROXIMITY = 0.92
...
def format_candidate_block(title: str, candidates: List[Candidate]) -> str:
    ...
    for c in candidates[:MAX_OUTPUT_PER_BLOCK]:
        ...
        lines.append(f"52주 신고가 근접도: {c.high_52w_proximity_pct:.2f}%" if c.high_52w_proximity_pct is not None else "52주 신고가 근접도: -")
